...
impl Ticket {
    pub fn new(
        ...
        if title.is_empty() {
            return Result::Err(TicketNewError::TitleError("Title cannot be empty".to_string()));
        }
        if title.len() > 50 {
            return Result::Err(TicketNewError::TitleError("Title cannot be longer than 50 bytes".to_string()));
        }
        if description.is_empty() {
            return Result::Err(TicketNewError::DescriptionError("Description cannot be empty".to_string()));
        }
        if description.len() > 500 {
            return Result::Err(TicketNewError::DescriptionError("Description cannot be longer than 500 bytes".to_string()));
        }

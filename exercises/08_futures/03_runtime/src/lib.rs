    ...
    T: Display + Send + Sync + 'static,
{
    ...
    let handle1 = tokio::spawn(_fixed_reply(first, Arc::clone(&arc_reply)));
    let handle2 = tokio::spawn(_fixed_reply(second, arc_reply));
    ...
    T: Display + Send + Sync + 'static,
{
    loop {
        ...
        writer.
            write_all(format!("{}", reply).as_bytes()).await.unwrap();
    }
}
